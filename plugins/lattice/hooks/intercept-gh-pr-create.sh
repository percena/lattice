#!/usr/bin/env bash
# PreToolUse: block bare `gh pr create` unless create-pr is active.
...
INTERCEPT_SYSTEM_MESSAGE="lattice: bare gh pr create blocked — /create-pr is the required path (strict)"
INTERCEPT_ADVICE='⚠️ Direct gh pr create detected. The create-pr skill is the required path, not an optional one.
