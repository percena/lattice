...
- **Evidence (re-verified at `e3e0872`, fix cycle 1):** `git --no-pager log e3e0872 --since='30 days ago' --format='%h %ad %s' --date=short | grep -E '^[0-9a-f]+ [0-9-]+ fix\([^)]*\): .*(flip (binder )?status|flip tkt-|status closed-open|backfill pr-open)'` → 10 commits: `3486e12` 08-27 (tkt-127), `2c5e3bf` 08-27 (tkt-131), `47379e3` 08-27 (tkt-143), `60e066a` 08-28 (tkt-150), `64a6abd` 08-28 (tkt-155), `c896957` 08-28 (tkt-174), `052de11` 08-31 (tkt-255), `d17e1ca` 09-02 (tkt-325/326), `45d18c8` 09-02 (tkt-327), `dd144ba` 09-02 (tkt-356 backfill). Not counted: `a878acb` (the `flip_close` predicate code fix that these repairs were waiting for), `abf8e0f` (binder format drift, not a status flip). Files in ≥ 2 `fix(` commits in 30 days: `skills/_lattice-lib/scripts/finish-ledger.sh` 15, `tools/validate-lattice-artifacts.py` 13, `finish-ledger.bats` 13, `stamp-pr-open.sh` 8, `skills/finish-work/SKILL.md` 8 (84 `fix(` commits in the window; 88 counting `fix:`-style subjects).
...
- **Mechanism:** `chmod +x` both + a mode lint in `tools/validate-skills.sh` scoped to **scripts a `SKILL.md` names as executables** (the same set `skill-scripts-exist` resolves — not every `skills/*/scripts/*` file: 16 files are legitimately `100644`, e.g. sourced `_lattice-home.sh`, `lib/*.py`, `transition-api.py`, `verify-mutation.sh`), so the probe's `high` row never fires for this class again (ticket 3).

### F4 — Three doc lines promise a `gh pr checks` field the script no longer requests (med · C4 · T2, T9 environment-dependence)

- **Failure scenario:** an agent following finish-work's flow or review-code's CI recipe runs `gh pr checks … --json name,state,conclusion,link` (`conclusion` is not a field on gh ≥ 2.6x; gh 2.92.0 here) and gets a field-mismatch error or reports the CI gate as broken; tkt-341 NOTICED it and tkt-349 fixed the script only.
- **Evidence (re-verified, fix cycle 1 — the probe's `--md` cell is truncated at 200 chars and showed only the first hit; re-running the registry command in full):** `grep -rnF --include='*.md' --exclude-dir=fixtures -- 'name,state,conclusion,link' skills docs` → **3 hits**: `skills/finish-work/references/flow.md:45` ("It fetches `gh pr checks <N> --json …`"), `skills/review-code/SKILL.md:122` ("PR mode: `gh pr checks <PR_N> --json …`"), `skills/review-code/references/ci-check.md:14` (the bare command). `skills/finish-work/scripts/ci-gate-check.sh:138-146` — docstring "`conclusion` is NOT a field … derive the legacy pair when absent".
- **Since last run:** new; the guard already exists (the probe row) — only the repair is missing.
- **Mechanism:** three one-line docs fixes (ticket 4); the probe keeps them fixed.
...
- **Evidence (re-verified at `e3e0872`, i.e. before this ticket's own binder added 4 lines):** `grep -l '^- NOTICED:' .lattice/tickets/*/README.md | wc -l` → 15 binders; dates in the lines: 08-27 ×4 (tkt-93/94), 08-31 ×2, 09-01 ×7, 09-02 ×16 (6 lines carry no date); `grep -ln NOTICED .lattice/reviews/*.md` → only `rev-20260826-141124Z` (design note, not a sweep).
...
| 2 | transition-api normalises ledger key to `tkt-N` + validator `ledger_key_not_ticket_id`; fold `356.jsonl`/`357.jsonl` into their ledgers | F2 | skills/_lattice-lib/scripts/transition-api.py, tools/validate-lattice-artifacts.py, .lattice/.transition-ledger/ | none | G1 | yes | fix | P2 | bind entries stranded outside the ticket's replay; land after draft 1 (both edit the validator) — `blocked_by` takes a real issue number once draft 1 is filed |
| 3 | chmod +x stamp-pr-open.sh / review-context.py + mode lint in validate-skills.sh for SKILL-named scripts only | F3 | skills/_lattice-lib/scripts/stamp-pr-open.sh, skills/review-code/scripts/review-context.py, tools/validate-skills.sh, tools/tests/ | none | G2 | yes | chore | P2 | SKILL-named scripts must be executable (skill-anatomy rule 1); lint scoped to the `skill-scripts-exist` set (16 `scripts/*` files are legitimately 100644) |
| 4 | drop `conclusion` from the documented `gh pr checks --json` field list (3 lines) | F4 | skills/finish-work/references/flow.md, skills/review-code/SKILL.md, skills/review-code/references/ci-check.md | none | G2 | yes | docs | P3 | three one-liners; `retired-paths-absent` probe already guards them |
...
**Ship plan:** multi-PR — G1 serial (draft 2 after draft 1, shared validator; `blocked_by` becomes `#<issue of draft 1>` at filing time — `create-tickets` grammar is `none | #N`), G2 ∥ G3 independent paths. **Needs-decision rows:** F6 — what a checked Spec `A*` must cite (`bats` name | `pr-N`/`tkt-N` | fixture id | nothing, evidence stays in binders); default: fold into the ADR-012 §6 follow-up Spec, keep the probe at `low`.
...
- **Corrections after review (fix cycle 1, re-verified at `e3e0872`):** F4 cited a truncated probe cell as "single hit" — the registry command re-run gives 3 hits; F1's commit regex was loose (missed `3486e12`, `d17e1ca`; included `abf8e0f`) — replaced by the exact regex + list above; `fix(` count 84 (not 88); `356.jsonl` was added by `e160dc6`/pr-359 (`357.jsonl` by `5fc6b63`/pr-358); `ensure-workspace.sh:670` (not :669); ticket 2 `blocked_by` uses `none` (grammar `none | #N`); ticket 3's lint scoped to SKILL-named scripts. All other cited numbers re-checked unchanged.
