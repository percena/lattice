...
- **Observability:** ticket state is currently smeared across issue state, PR existence, markers, worktrees, and binder prose. Add a frontmatter **`status:` FSM field** on binders (queued / in-progress / parked / stuck / pr-open / rework / merged / cancelled / deferred): validators can reject illegal transitions, the digest becomes a fold over binder states, and time-in-state falls out as the first DORA-lite metric for free.
