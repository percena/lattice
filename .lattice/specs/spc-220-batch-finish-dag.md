...
- [ ] **A1** `merge_blocked_by` field exists in the ticket-binder template + create-tickets policy, documented as merge-order (not work-start), with fallback-to-`blocked_by` noted.
...
- [ ] **A4** Two PRs where B `merge_blocked_by A` land in order L0=[A], L1=[B]; after A merges, B's `update-pr-base` pulls A's work; if B's `baseRefName` was A's branch, the retarget to the integration branch fires; B's diff cleans; B merges; ledger stamped.
