...
9. `finish-ledger.sh --pr <N> --issue <closing_M> --binder <path>` on the merge base, once per closing binder. Stamps `pr-open → closed` + `## Finish` ledger line; the helper writes + stages the binder + its `.transition-ledger/<tkt>.jsonl` (does **not** commit/push itself). **tkt-360 A1:** fails closed if the ledger entry is not staged after a flip (gitignored ledger, held index lock, foreign cwd). After the per-binder loop, the flow commits + pushes the base once: `finish-commit.sh --message "finish(tkt-…): …"` (tkt-360 A2 — asserts `git status --porcelain -- .lattice` empty post-commit) + `git push origin <base>`. **Halt-on-failure:** if finish-ledger fails on binder k of N, do **not** commit the partial staged set — resolve and re-run.
