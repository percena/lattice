...
- Material findings → `AskUserQuestion` with **dynamic ordering** (per `../_lattice-lib/references/confirmation-ux.md`): tag the severity-default option `(Recommended)` and list it first at presentation time. Any **high** finding — including credential/secret leak → present `Hold (I'll address)` first as `(Recommended)`; only med/low → present `Merge anyway` first as `(Recommended)`. The three options: `Merge anyway` / `Hold (I'll address)` / `Invoke full /review-code`. **Privacy/Secrets override:** privacy **high** (credentials, API keys, private keys) defaults to `Hold` regardless of other axes; **medium** (local paths, project names) → recommend cleanup, allow `Merge anyway` after explicit confirmation.
...
- [ ] Mini-review scan ran (default-on): material findings → `AskUserQuestion` with the severity-default option tagged `(Recommended)` and first (dynamic ordering, `../_lattice-lib/references/confirmation-ux.md`); advice not gate; no auto-fix
