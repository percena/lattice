...
if $CREATED_CONFIG || $CREATED_README || $CREATED_DOTLATTICE_GITIGNORE; then
...
if $AS_JSON; then
  python3 - "$LATTICE" "$CREATED_CONFIG" "$CREATED_README" "$CREATED_DOTLATTICE_GITIGNORE" "$GITIGNORE_WROTE" "$LABELS_RAN" "$LABELS_MSG" "$ACTIVE_PROFILE" "$ROOT" <<'PY'
import json, sys
lattice, cc, cr, dlg, gw, lr, lm, prof, root = sys.argv[1:10]
print(json.dumps({
  "ok": True,
  "root": root,
  "lattice": lattice,
