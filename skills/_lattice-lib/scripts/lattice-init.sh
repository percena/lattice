...
# Lattice ignore block (inline). Previously materialized as
# .lattice/gitignore.snippet, which leaked into consumer git status when its
# self-ignore line had not yet been merged into .gitignore (loose ALREADY
# heuristic, partial/older .gitignore, or direct lattice-init.sh without
# --write-gitignore). The block now stays inline — no persistent file.
lattice_ignore_block() {
  cat <<'EOF'
...
PY
  ...
  if ! $WRITE_GITIGNORE; then
    echo "  tip: merge the block below into .gitignore, or re-run with --write-gitignore"
    lattice_ignore_block
  fi
