...
@test "gh --set-parent path when flag present" {
  ...
  [ "$status" -eq 0 ]
  [[ "$output" == *"--set-parent"* || "$output" == *"linked via gh --set-parent"* ]]
...
@test "falls back to --add-sub-issue" {
  ...
  run bash "$ADD" --parent 10 --child 11
  [ "$status" -eq 0 ]
  [[ "$output" == *"linked via gh --add-sub-issue"* ]]
...
@test "REST fallback sends sub_issue_id as typed -F field (integer, not string)" {
  ...
  ! grep -F -e "-f sub_issue_id=" "$GH_LOG"
}
...
@test "URL forms parse numbers" {
  ...
  [ "$status" -eq 0 ]
  [[ "$output" == *"linked via gh --set-parent"* ]]
...
@test "all failure modes still exit 0" {
  ...
  [ "$status" -eq 0 ]
  [[ "$output" == *"non-fatal"* || "$output" == *"failed"* || "$output" == *"skip"* || "$output" == *"already"* || "$output" == *"linked"* || "$output" == *"GraphQL"* || "$output" == *"REST"* || "$output" == *"gh issue edit failed"* || "$output" == *"parent link failed"* ]]
}
...
@test "PR URL is refused — trailing digits never parsed as an issue number" {
  ...
  [ "$status" -eq 0 ]
  [[ "$output" == *"not a GitHub /issues/ URL"* ]]
  [[ "$output" == *"cannot parse parent issue number"* ]]
  [ ! -s "$GH_LOG" ]
}
...
@test "discussions URL is refused like a PR URL" {
  ...
  [ "$status" -eq 0 ]
  [[ "$output" == *"not a GitHub /issues/ URL"* ]]
  [[ "$output" == *"cannot parse child issue number"* ]]
  [ ! -s "$GH_LOG" ]
}
...
@test "ambiguous already-message is logged as ambiguous, never success" {
  ...
  [ "$status" -eq 0 ]
  [[ "$output" != *"already linked"* ]]
  [[ "$output" == *"parent link attempt ambiguous: link already pending review"* ]]
}
...
@test "cross-repository parent/child URLs are refused before mutation" {
  ...
  [ "$status" -eq 0 ]
  [[ "$output" == *"cross-repository parent/child refused"* ]]
  # soft-fail exit 0; no mutation flags
  if [[ -f "$GH_LOG" ]]; then
    ! grep -E "set-parent|add-sub-issue|sub_issues|graphql" "$GH_LOG"
  fi
}
