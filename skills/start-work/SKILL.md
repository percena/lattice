...
7. EXECUTE under the accountable owner **unless** setup-only → stop with `/start-work tkt-N` hint. Bounded delegation is allowed.
   ...
   - **`--budget <min>,<retries>` (spc-433; semantics fixed by spc-458 D2):** the **per-ticket** outer bound over the `batch_timebox_*` inner timebox (DEFAULT 60 min / 5 retries, overridable per run). The retry count is the `## Attempts` ledger entries for this ticket across sessions (the same counter `fallback-policy.md` caps), so a resumed ticket does not restart the budget. At trip: stamp `status: stuck` + `wait_reason: unblock` via `transition-api.py` (`in-progress → stuck`, the watchdog edge — **not** `deferred`/`budget-exhausted`, which is batch-work's *per-batch* ceiling for never-spawned tickets); write `.lattice/blocked/<tkt>-debug-dump.json` (context, last error, attempt count); open a `[BLOCKED]` **draft PR via `create-pr`** carrying the dump; `git clean` the tree. The `## Attempts` ledger + status FSM + transition ledger are the durable failure handoff — no new snapshot mechanism. This is instruction-shaped (start-work has no scripts): the agent watches its own wall-clock and attempt count.
   - **`--unattended` (spc-433):** activates the decision-policy.md resolution chain as the **total function** (resolve-or-park, never block). Operating rule injected into the working context: *"On ambiguity choose the highest-confidence option yourself; mark it in code with `// Auto-Decided: <reason>` (`# Auto-Decided:` for Python/Shell), aggregate all such decisions in a `## Auto-Decided` PR-body section, and journal each per decision-policy.md (promote to an ADR only when cross-feature). Never stop to wait for user input."* The SKILL.md rule "Non-interactive / CI: do not invent PCA answers — fail closed" becomes the active mode, not an edge case. The binder `autonomy` field (0-4, spc-433) scopes which `## Anticipated decisions` items may be `agent-decides`: items on tickets with autonomy < 2 are upgraded to `must-ask` and parked. All `Auto-Decided` actions are journaled per decision-policy.md.
...
- Non-interactive / CI: do not invent PCA answers — fail closed or setup-only. **`--unattended` (spc-433):** activates decision-policy.md resolution chain (resolve-or-park, never block); `Auto-Decided:` comments + decision-journal entries replace blocking prompts. Autonomy field (0-4) scopes which items may be `agent-decides`.
