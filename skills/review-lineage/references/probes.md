...
| `spec-done-acceptance-cites-evidence` | In every `status: done` Spec under `<home>/specs/`, each checked `- [x] **A<n>**` item (bullet + indented continuation lines, case-insensitive) mentions `bats`, `test`, `ci`, or a `pr-<N>` / `tkt-<N>` id — else the A<n> ids are listed. Stricter than the current Spec convention (evidence conventionally lives in binders / PR bodies, not on the Spec line), hence severity `low`: an audit signal, not a gate (rev-20260902-015425Z F6: `done` is self-reported; ADR-007 §3 decidability) | `[ -d "$LATTICE_HOME/specs" ] \|\| { echo "skip: no specs dir under $LATTICE_HOME"; exit 3; }; for f in "$LATTICE_HOME"/specs/*.md; do [ -f "$f" ] \|\| continue; grep -qE '^status: done' "$f" \|\| continue; awk -v n="$(basename "$f")" 'function flush(){ if (cur != "") { l=tolower(cur); if (l !~ /bats\|test\|(^\|[^a-z])ci([^a-z]\|$)\|pr-[0-9]+\|tkt-[0-9]+/) { match(cur, /\*\*A[0-9]+/); ids=ids (ids==""?"":" ") substr(cur, RSTART+2, RLENGTH-2) } } cur="" } /^- \[x\] \*\*A[0-9]+/{flush(); cur=$0; next} /^[ \t]+[^ \t]/ && cur != "" {cur=cur " " $0; next} {flush()} END{flush(); if (ids != "") print n ": " ids " — no test/PR/ticket evidence cited"}' "$f"; done` | `empty` | low |
