...
Registry: `references/probes.md` (`id | claim (where) | probe | expect | severity`); per-repo overlay `<home>/lineage-probes.tsv` merged by id. A `fail` row is a **candidate**: the evidence cell is truncated to 200 chars, so reconciliation means re-running the probe's one-liner from the registry (cwd = repo root, `LATTICE_HOME`/`REGISTRY_DIR` exported) to get the full list, then opening the promise (doc line, ADR §, Spec A*) and the implementation at the exact point. Decide which side is wrong; both outcomes are findings. A `skip` row with a reason is not a pass — note it in Method as an unaudited surface.
