...
2. **Launch agent** (background):
   ...
   - Prompt brief (bounded delegation — the **Spawn-brief contract**, SKILL.md; every numbered block below is mandatory when applicable):
     ...
     ```
   - Before spawning the FIRST agent, define the batch id ONCE and create the batch marker at the out-of-repo state home via the gate helper (single gate point, spc-186 A1, ADR-011 / spc-282 A1; scripted creation per spc-337 A6):
     ```bash
     BATCH_ID="$(date -u +%Y%m%d)-$$"
     bash "$SKILL_ROOT/../finish-work/scripts/batch-merge-gate.sh" --create --batch-id "$BATCH_ID"
     ```
     `--create` writes the `batch-id:` + `started:` lines, is idempotent for the same id, and REFUSES a marker owned by a different batch-id unless `--force` (a leftover marker means a batch is live or was not finished — STOP and triage via `--status`; never silently overwrite). Do NOT write per-worktree copies and do NOT hand-`printf` the marker. Reuse the same `$BATCH_ID` for every `run-process-wave.sh --batch-id` call in this batch. The marker lives OUT of the repo tree at `$XDG_STATE_HOME/lattice/<repo-fingerprint>/` (keyed by sha1(git common-dir)[:12]) so it never leaks as untracked dirt in a fresh customer repo; the merge hook resolves the same state home, so the gate stays fail-closed.
...
2. Call:
   ...
   bash skills/batch-work/scripts/run-process-wave.sh \
     ...
     --state-file <batch-state> \
     --batch-id "$BATCH_ID" --layer <L> --wave <W> [--report <path>]
   ```
   `--batch-id "$BATCH_ID"` (the marker's id from SPAWN LAYER) is part of the canonical call, not an option: it activates the coordinator spine (`scripts/lib/coordinator.py` persists DAG/layer/node-attempt/PID-PR-OID/failure-class/resume-cursor at `<state-home>/.coordinator/<batch-id>.json`, so a host restart resumes from the cursor without re-deriving — spc-254 A5 / spc-270 A3 / spc-337 A6) and the per-barrier marker heartbeat (the wave runs `batch-merge-gate.sh --touch` when each barrier settles, ADR-011). `--coordinator <path>` only overrides which coordinator script the spine uses; it never activates it. The script spawns `claude --bg -p "$(cat brief_file)" --permission-mode acceptEdits` detached per worktree (via `spawn-ticket-process.sh`, which fixes the reference tool's shared-`cwd` defect by binding `cwd` to each ticket's sibling worktree — ADR-006 preserved), records pid+worktree+started-iso to the batch state file, and runs the barrier (below).
3. The batch marker (`.batch-work-active` at the out-of-repo state home) is created via `batch-merge-gate.sh --create --batch-id "$BATCH_ID"` before the first wave in **both** modes — process mode is not exempt. In process mode the wave keeps it alive: each barrier ends with `batch-merge-gate.sh --touch` (mtime heartbeat) so the stale-marker GC never reaps a live batch.
...
- **Watchdog:** each ticket's `spawn timestamp` (recorded in the batch state file) is compared to wall-clock at each poll; `elapsed > timebox` → kill the pid, mark `timeout`, and the wave stamps the binder `status: stuck` + `wait_reason: unblock` via `transition-api.py commit` (FSM-2b, tkt-132; through the coordinator when `--batch-id` is set, else directly). Same FSM as `agent` mode — the mode flag only selects the liveness source.
- **Classification (spc-254 A1 — false-success closure):** when a PID dies within its timebox the barrier does NOT mark it `completed`; it classifies the node from FOUR signals and stamps a final state `ok | failed | timeout | unknown`. A PID that disappeared is NEVER success.
  ...
  - `ok` requires exit==0 AND a PR claim AND verify verified AND agents not-failed (agreement across the available signals). `failed` = exit!=0 OR phantom PR OR agents explicit failure. `unknown` = no result artifact (PID disappeared) OR exit==0 but no verified PR claim (can't confirm success). `failed|timeout|unknown` **ALL fail-close the binder to `stuck + wait_reason: unblock`** via `transition-api.py commit` BEFORE the node settles (tkt-255; spc-337 A6 added `failed` — a worker that crashed or claimed a phantom PR after `start-work` stamped `in-progress` must never leave the binder reading "active work"); a refused transition leaves the node unsettled and the wave exit non-zero (spc-270 A2.3 / A3.4). The host triages the stuck node. This closes the F1 gap where a worker that ran long, then failed without opening a PR, was first marked `completed`.
