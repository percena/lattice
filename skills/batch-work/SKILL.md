...
| Binder `status` is stamped | Spawned tickets flip binder `status` `queued → in-progress → pr-open` (agents stamp it per their brief; enum + validator per the ticket-binder template). Fuse-halted tickets stamp `deferred` + reason `fuse-halt` at trip time (ADR-004 amd tkt-136 Option B); blocked-by-failure dependents stamp `deferred` + reason `blocked-by-failure`; watchdog-timeout stamps `stuck` + `wait_reason: unblock` (FSM-2b, tkt-132); never-spawned tickets stay `queued` with a report note — no new enum values |
...
- [ ] Binder `status` stamped `queued → in-progress → pr-open` per spawned ticket; fuse-halted tickets stamped `deferred`+reason; watchdog-timeout stamped `stuck`+`wait_reason`; never-spawned tickets left `queued` with a report note (no invented enum values)
