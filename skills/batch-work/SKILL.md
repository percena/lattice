...
argument-hint: "[--ids ID1,ID2,... | --groups] [--spawn-mode {agent,process}] [--concurrency N] [--ram-threshold <GB>] [--min-autonomy <0-4>] [--budget <minutes>,<retries>] [--base <ref>] [--report <path>] [--dry-run] [--with-review]"
metadata:
...
| Batch marker blocks merge | Before spawning the first agent, the orchestrator writes ONE `.batch-work-active` marker at the out-of-repo state home (`$XDG_STATE_HOME/lattice/<repo-fingerprint>/`, single gate point — NOT per-worktree; retiring per-worktree copies is the spc-186 A1 decision; relocated out-of-repo by ADR-011 / spc-282 A1 so it never leaks as untracked dirt in a fresh customer repo). The merge hook (plugins/lattice/hooks/lib/batch-merge-gate.sh) blocks `gh pr merge` while the marker exists; finish-work removes it as a deliberate scripted step BEFORE merge (after human ack). Agents stop at `create-pr` (PR opened, human reviews, then human runs finish-work per PR). The marker carries a batch-id + timestamp line |
...
| Autonomy filter | `--min-autonomy <level>` (DEFAULT 3) filters tickets whose binder `autonomy` field is below the threshold from night-batch execution. Filtered tickets stay `queued` with never-spawned reason `autonomy-below-threshold` — they are not failed, just deferred to day-interactive. Tickets without an `autonomy` row default to 2 (medium) per `autonomy-rubric.md` — below default threshold, so legacy binders are filtered unless `--min-autonomy 0` overrides. The filter runs at RESOLVE TICKETS, before BUILD DAG |
| Budget circuit breaker | `--budget <minutes>,<retries>` (DEFAULT 60,5 per spc-433 decision 2) sets a per-batch wall-clock + retry ceiling. At each layer barrier, if cumulative elapsed > budget minutes OR total retries across tickets > retry ceiling → trip: halt subsequent layers, graceful-drain in-flight agents, stamp never-spawned tickets `deferred` + `wait_reason: budget-exhausted`, emit partial report. The law is `fallback-policy.md` §Budget circuit breaker; wiring is in `flow.md` FUSE CHECK |
| Binder `status` is stamped | Spawned tickets flip binder `status` `queued → in-progress → pr-open`, but **never by agent hand-prose** (ADR-012 §1): the `ensure-workspace --bind tkt` bind stamps `queued → in-progress` at spawn (a second `commit … in-progress` is refused — no `in-progress → in-progress` edge), and `after-pr-open.sh` (create-pr step) / the PostToolUse `auto-stamp-pr-open.sh` hook stamps `pr-open` after `gh pr create` (the L3 status-row hook refuses a hand edit). Fuse-halted tickets stamp `deferred` + `wait_reason: fuse-halt` at trip time (ADR-004 amd tkt-136 Option B); blocked-by-failure dependents stamp `deferred` + `wait_reason: blocked-by-failure`; watchdog-timeout stamps `stuck` + `wait_reason: unblock` (FSM-2b, tkt-132). Never-spawned tickets have six mutually-exclusive reasons (see `references/flow.md` §Never-spawned reason mapping): `not-selected`, `workspace-failed`, and `autonomy-below-threshold` leave the binder `queued` (still schedulable); `fuse-halted`, `budget-exhausted`, and `blocked-by-failure` stamp `deferred` (not schedulable until a human re-queues). No new enum values |
...
| `--min-autonomy 3` night filter | `--min-autonomy 0` disables; tickets below threshold stay `queued` (day-interactive) |
| `--budget 60,5` (60min, 5 retries) | `--budget 0,0` disables; spc-433 decision 2 default |
...
| 6 | Verify-after-mutate | A `gh pr create` / `gh pr merge` / `git push` that reported success but left no durable artifact is the highest-cost silent failure (rev-20260829-140444Z F5). After every such mutation, run `skills/_lattice-lib/scripts/verify-main-chain.sh --stage push|pr|merge …` (spc-254 A2/D5 — one shared helper contract; push: capture local HEAD before push, then `--stage push --branch <name> --expected-oid <HEAD>`; pr: `--stage pr --pr <N> --expected-oid <HEAD> --repo <owner/name> [--expected-base …]`; merge: capture base tip before merge, then `--stage merge --pr <N> --expected-oid <base tip>`) and include the `verified:`/`FAILED:` line in your report. On `FAILED`, **do not proceed on assumed success** — stamp `stuck` + `wait_reason: unblock` and stop. **Process mode (spc-254 A1):** the wave exports `$BATCH_RESULT_FILE` (a per-ticket path) before spawn; the worker MUST write its exit/result there — `exit=<int>` plus, when a PR was opened, `pr=<N>` and `oid=<HEAD>` (the same head OID verify-main-chain confirmed). The wave reads this at the barrier to classify `ok|failed|timeout|unknown`; an absent artifact means the PID disappeared without a result → `unknown` fail-close (never success). |

The brief also carries the per-ticket timebox, the binder `status` stamping note (the `ensure-workspace` bind already stamped `in-progress` at spawn — agents do **not** hand-stamp `status`; `after-pr-open.sh` stamps `pr-open` after `create-pr`), and — for stacked layers — the base ref, the Stacking note for the PR body, and the interface contracts (exact file/section names) the ticket depends on.
...
7. **SPAWN LAYER (waves)** — if the layer has more tickets than `--concurrency`, spawn in **waves** of `--concurrency` tickets each, with a barrier between waves (see step 8). Within a wave:
   ...
   - Launch the ticket per `--spawn-mode` (ADR-008). Before spawning the FIRST ticket in the batch, define the batch id ONCE — `BATCH_ID="$(date -u +%Y%m%d)-$$"` — and create the batch marker at the out-of-repo state home via the gate helper (single gate point, ADR-011 / spc-282 A1; scripted creation per spc-337 A6): `bash "$SKILL_ROOT/../finish-work/scripts/batch-merge-gate.sh" --create --batch-id "$BATCH_ID"` (writes `batch-id:` + `started:` lines; idempotent for the same id; refuses a different id without `--force` — a stale marker from another batch means STOP and triage, not overwrite). Do NOT write per-worktree copies and do NOT hand-`printf` the marker; it lives at `$XDG_STATE_HOME/lattice/<repo-fingerprint>/`, keyed by sha1(git common-dir)[:12] so all sibling worktrees resolve one marker. The same `$BATCH_ID` is passed to `run-process-wave.sh --batch-id` below so the coordinator spine and the marker heartbeat are keyed by one id. Brief: full **Spawn-brief contract** template (`references/flow.md` §SPAWN LAYER) — core instruction stays "run `start-work tkt-<id>` in worktree <path>; implement to acceptance; open PR via `create-pr`; do NOT call `finish-work`", plus the six contract items, the timebox, and the binder-status stamping instruction.
     ...
     - `process`: Write the brief to a file, then per wave call `bash skills/batch-work/scripts/run-process-wave.sh --manifest <wave-manifest> --concurrency <N> --ram-threshold <GB> --state-file <batch-state> --batch-id "$BATCH_ID" --layer <L> --wave <W> [--report <path>]` (`references/flow.md` §PROCESS-MODE SPAWN + §PROCESS-MODE STATUS DETECTION). `--batch-id` is NOT optional in the canonical path: it activates the coordinator spine (persisted DAG/cursor/attempts so a host restart resumes without re-deriving — spc-270 A3, spc-337 A6) and the per-barrier marker heartbeat (`batch-merge-gate.sh --touch`, ADR-011). The wave manifest is TSV `ticket<TAB>worktree<TAB>brief_file<TAB>timebox_min`; worktrees are ensure-workspace'd first. The script spawns `claude --bg` detached per worktree (via `spawn-ticket-process.sh`), polls `kill -0` + per-ticket timebox, and prints a compact report — the host's context is not flooded by N completion reports. Per spc-254 A1 the wave classifies each settled node `ok|failed|timeout|unknown` IN-WAVE from exit/result artifact (`$BATCH_RESULT_FILE`) + `claude agents --json` + PID + `verify-mutation --expected-oid`; `failed|timeout|unknown` ALL fail-close the binder to `stuck + wait_reason: unblock` via `transition-api.py commit` before the node settles (spc-337 A6 — a crashed worker never leaves its binder `in-progress`). The host no longer re-probes `completed` tickets — a row is `ok` only after the in-wave verify-mutation probe agrees.
...
8. **LAYER/WAVE BARRIER + WATCHDOG** — wait for all tickets in the current wave to settle (success or fail). A ticket exceeding its timebox is marked `failed` (`timeout`) and no longer waited on; its binder keeps whatever ledger exists. Collect exit status + PR URL per ticket. In `process` mode the wave script's `ok`/`failed`/`timeout`/`unknown` rows ARE this barrier's result; in `agent` mode the in-session background-completion channel is the result. **FUSE CHECK:** compute the layer's cumulative failed+stuck ratio (`unknown` counts as stuck); over threshold → halt all subsequent waves/layers, graceful-drain in-flight agents (finish current attempt, write ledgers), jump to REPORT with partial results. Otherwise, if more waves remain in this layer, run RAM check again and spawn the next wave. A failed ticket is recorded; peers and subsequent waves/layers proceed (failure isolation).
...
12. **HANDOFF** — summary: N spawned, M ok, K failed, L blocked (+ fuse trip if any). Human reviews open PRs (digest-first when `--with-review`), then runs `finish-work` per PR. The out-of-repo `.batch-work-active` marker (state home) ensured no agent merged.

Detailed recipes (DAG build, RAM probe, spawn/collect, report shape): **`references/flow.md`**.

## Safety

- **Batch marker merge block.** Before spawning the first agent, the orchestrator writes ONE `.batch-work-active` marker at the out-of-repo state home (`$XDG_STATE_HOME/lattice/<repo-fingerprint>/`, single gate point — not per-worktree; spc-186 A1 retires per-worktree copies; ADR-011 / spc-282 A1 relocates it out-of-repo). The merge hook (plugins/lattice/hooks/lib/batch-merge-gate.sh) blocks `gh pr merge` while the marker is present, keeping a human review gate over the batch. Agents are instructed not to call `finish-work`. **The marker is removed by finish-work as a deliberate scripted step BEFORE merge** (after human ack, via `batch-merge-gate.sh --remove --reason "..."`), not after the merge — the merge hook enforces this fail-closed.
...
- A spawned worktree without the out-of-repo `.batch-work-active` marker (single gate point at the state home), or any agent invoking merge/`finish-work` during the batch window
...
- [ ] Orchestrator created ONE `.batch-work-active` marker at the out-of-repo state home via `batch-merge-gate.sh --create --batch-id "$BATCH_ID"` (batch-id + started lines; no raw `printf`); no per-worktree copies; no agent called `finish-work`
- [ ] Every `run-process-wave.sh` call carried `--batch-id "$BATCH_ID"` (same id as the marker); the coordinator state file exists at the state home; the marker mtime advanced at each barrier (heartbeat via `batch-merge-gate.sh --touch`)
...
- [ ] Per-ticket timebox rode each brief; watchdog marked over-timebox tickets `failed` (`timeout`); `failed|timeout|unknown` ALL fail-closed the binder to `stuck` + `wait_reason: unblock` at settle time (via `transition-api.py commit`, before the node settled), and left the binder ledger intact
...
- [ ] Binder `status` stamped `queued → in-progress → pr-open` per spawned ticket **by path points, not agent hand-prose** (`ensure-workspace` bind → `in-progress`; `after-pr-open.sh` → `pr-open`); fuse-halted/blocked-by-failure/budget-exhausted stamped `deferred`+`wait_reason`; watchdog-timeout stamped `stuck`+`wait_reason`; the six never-spawned reasons (`not-selected`/`workspace-failed`/`autonomy-below-threshold`→`queued`, `fuse-halted`/`budget-exhausted`/`blocked-by-failure`→`deferred`) each map to exactly one binder state (no invented enum values)
