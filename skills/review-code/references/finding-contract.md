...
3. Likely impact?
4. Concrete recommendation? (direction, not necessarily a patch)
...
Before finalizing: empty/null paths, retries/partial failure/idempotency, stale state/ordering, rollback/irreversible writes.
...
| Table | Sev · Finding · Failure scenario · Evidence · Confidence · Recommendation |
...
After presenting findings: **STOP**. Do not auto-fix. Edit only if the user explicitly names findings or asks for tests/fixes.
